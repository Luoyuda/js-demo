 ...
 * @LastEditTime: 2020-06-24 09:57:18
 * @FilePath: /js-demo/data-structures/Stack/Stack.test.js
 */ 
const { Stack, StackLinked } = require('./Stack');
const { expect } = require('chai');

describe('栈(数组实现)', function() {
    it('检查空栈', () => {
        const stack = new Stack();
        expect(stack).to.not.null;
        expect(stack.list).to.not.null;
    });
    ...
    it('入栈简单类型操作', () => {
        ...
        stack.push(true);
        stack.push(0.1);
        expect(stack.toString()).to.equal('1,2,false,true,0.1');
        expect(stack.toArray()).to.eql([1,'2',false,true,0.1]);
    });
    ...
    it('入栈复杂类型', () => {
        ...
        expect(stack.toString(stringifier)).to.equal('key1:test1,key2:test2');
