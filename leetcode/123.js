/*
 * @Author: xiaohuolong
 * @Date: 2021-03-15 22:29:49
 * @LastEditors: xiaohuolong
 * @LastEditTime: 2021-03-15 23:33:19
 * @FilePath: /js-demo/leetcode/123.js
 */
/**
 * @param {number[]} prices
 * @return {number}
    123. 买卖股票的最佳时机 III
        给定一个数组，它的第 i 个元素是一支给定的股票在第 i 天的价格。
        设计一个算法来计算你所能获取的最大利润。你最多可以完成 两笔 交易。
        注意：你不能同时参与多笔交易（你必须在再次购买前出售掉之前的股票）。
    示例 1:
        输入：prices = [3,3,5,0,0,3,1,4]
        输出：6
        解释：在第 4 天（股票价格 = 0）的时候买入，
        在第 6 天（股票价格 = 3）的时候卖出，这笔交易所能获得利润 = 3-0 = 3 。
        随后，在第 7 天（股票价格 = 1）的时候买入，
        在第 8 天 （股票价格 = 4）的时候卖出，这笔交易所能获得利润 = 4-1 = 3 。
    示例 2：
        输入：prices = [1,2,3,4,5]
        输出：4
        解释：在第 1 天（股票价格 = 1）的时候买入，在第 5 天 （股票价格 = 5）的时候卖出, 这笔交易所能获得利润 = 5-1 = 4 。   
            注意你不能在第 1 天和第 2 天接连购买股票，之后再将它们卖出。   
            因为这样属于同时参与了多笔交易，你必须在再次购买前出售掉之前的股票。
    示例 3：
        输入：prices = [7,6,4,3,1] 
        输出：0 
        解释：在这个情况下, 没有交易完成, 所以最大利润为 0。
    示例 4：
        输入：prices = [1]
        输出：0
    提示：
        1 <= prices.length <= 105
        0 <= prices[i] <= 105

 */
// 递归
var maxProfit = function(prices) {
    if(prices==null || prices.length==0) {
        return 0;
    }
    let n = prices.length;
    let map = {}
    /**
     * 遍历
     * @param {*} prices 
     * @param {*} index 第几天
     * @param {*} status 当前状态
     * @param {*} k 交易次数
     */
    let dfs = (prices, index, status, k) => {
        let key = `${index}-${status}-${k}`
        if(map[key]) return map[key]
        // 达到交易次数 或 走到最后一天
        if(k == 2 || index == prices.length) return 0
        let wait = 0
        let buy = 0
        let sell = 0
        // 状态保持不动
        wait = dfs(prices, index+1, status, k)
        if(status == 1){
            // 卖出的情况
            sell = dfs(prices, index+1, 0, k + 1) + prices[index]
        }else{
            // 买入
            buy = dfs(prices, index+1, 1, k) - prices[index]
        }
        map[key] = Math.max(wait, sell, buy)
        return map[key]
    }
    return dfs(prices,0,0,0)
};

// 3维数组
var maxProfit = function(prices) {
    let dp = []
    let n = prices.length
    prices.forEach((item) => {
        dp.push([[],[],[]])
    })
    dp[0][0][0] = 0
    dp[0][0][1] = -prices[0]
    dp[0][1][0] = 0
    dp[0][1][1] = -prices[0]
    dp[0][2][0] = 0
    dp[0][2][1] = -prices[0]
    for (let i = 1; i < n; i++) {
        const price = prices[i];
        dp[i][0][0] = dp[i-1][0][0]
        // 第1次买入
        dp[i][0][1] = Math.max(dp[i - 1][0][1], dp[i-1][0][0] - price)
        // 第1次卖出
        dp[i][1][0] = Math.max(dp[i - 1][1][0], dp[i-1][0][1] + price)
        // 第2次买入
        dp[i][1][1] = Math.max(dp[i - 1][1][1], dp[i-1][1][0] - price)
        // 第2次卖出
        dp[i][2][0] = Math.max(dp[i - 1][2][0], dp[i-1][1][1] + price)
    }
    // console.log(dp)
    return Math.max(dp[n - 1][0][0], dp[n - 1][0][1], dp[n - 1][1][0], dp[n - 1][1][1], dp[n - 1][2][0])
}

// 2维数组
var maxProfit = function(prices) {
    let dp = []
    let n = prices.length
    prices.forEach((item) => {
        dp.push([])
    })
    dp[0][0] = 0
    dp[0][1] = -prices[0]
    dp[0][2] = 0
    dp[0][3] = -prices[0]
    dp[0][4] = 0
    dp[0][5] = -prices[0]
    for (let i = 1; i < n; i++) {
        const price = prices[i];
        dp[i][0] = dp[i-1][0]
        // 第1次买入
        dp[i][1] = Math.max(dp[i - 1][1], dp[i-1][0] - price)
        // 第1次卖出
        dp[i][2] = Math.max(dp[i - 1][2], dp[i-1][1] + price)
        // 第2次买入
        dp[i][3] = Math.max(dp[i - 1][3], dp[i-1][2] - price)
        // 第2次卖出
        dp[i][4] = Math.max(dp[i - 1][4], dp[i-1][3] + price)
    }
    console.log(dp)
    return Math.max(...dp[n - 1])
}

// 变量
var maxProfit = function(prices) {
    let n = prices.length
    let dp1 = 0
    let dp2 = -prices[0]
    let dp3 = 0
    let dp4 = -prices[0]
    let dp5 = 0
    for (let i = 1; i < n; i++) {
        const price = prices[i];
        // 第1次买入
        dp2 = Math.max(dp2, dp1 - price)
        // 第1次卖出
        dp3 = Math.max(dp3, dp2 + price)
        // 第2次买入
        dp4 = Math.max(dp4, dp3 - price)
        // 第2次卖出
        dp5 = Math.max(dp5, dp4 + price)
    }
    console.log(dp1, dp2, dp3, dp4, dp5)
    return Math.max(0, dp2, dp3, dp4, dp5)
}

console.log(maxProfit([7,1,5,3,6,4]))
console.log(maxProfit([7,1,5,3]))