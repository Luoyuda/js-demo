/*
 * @Author: xiaohuolong
 * @Date: 2021-08-12 10:30:01
 * @LastEditors: xiaohuolong
 * @LastEditTime: 2021-08-12 10:31:12
 * @FilePath: /js-demo/leetcode/常规题目/642.js
 */
/*
642. 设计搜索自动补全系统
...
class Trie {
    ...
    search(word){
        ...
        }).map(a => a.word).slice(0,3)
    }
    dfs(node, res){
        if(node.isEnd) res.push(node)
        for(let key of Object.keys(node)){
            if(['isEnd', 'times', 'word'].includes(key)) continue
            this.dfs(node[key], res)
        }
    }
}
/**
...
var AutocompleteSystem = function(sentences, times) {
    ...
    sentences.forEach((item, index) => {
        this.trie.insert(item, times[index])
    })
};
...
AutocompleteSystem.prototype.input = function(c) {
    ...
    this.cur += c
    return this.trie.search(this.cur)
};
