/*
 * @Author: xiaohuolong
 * @Date: 2021-04-05 11:39:41
 * @LastEditors: xiaohuolong
 * @LastEditTime: 2021-04-05 11:53:32
 * @FilePath: /js-demo/leetcode/常规题目/1370.js
 */
/**
 * @param {string} s
 * @return {string}
...
var sortString = function(s) {
    ...
    for (let i = 0; i < s.length; i++) {
        let code = s[i].charCodeAt() - start
        num[code] += 1
        max = Math.max(max, code)
    }
    num = num.slice(0, max + 1)
    let ret = ''
    while (ret.length < s.length){
        for (let i = 0; i < num.length; i++) {
            if(num[i]){
                // console.log(String.fromCharCode(i + start))
                ret += String.fromCharCode(i + start)
                num[i]--
            }
        }
        // console.log(num)
